...
export function Session({
  ...
  return (
    <div className={className}>
      <div
        ref={wrapRef}
        className="overflow-hidden rounded-xl p-3 sm:p-4"
...
function Key({ label, help, onClick }: { label: string; help: string; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="group inline-flex items-center gap-2 rounded-full px-3 py-1.5 font-mono text-ink-muted transition-colors hover:text-ink"
      style={{ boxShadow: "inset 0 0 0 1px var(--hairline)" }}
    >
      <span className="text-ink">
        <span className="text-ink-faint">C-s</span> {label}
      </span>
      <span className="hidden text-ink-faint group-hover:text-ink-muted sm:inline">{help}</span>
    </button>
  );
}
