...
export function Agents() {
  return (
    ...
    >
      <div className="mt-12 grid gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,0.85fr)] lg:gap-14">
        <div>
          <ul className="text-[15px]">
            {COMMANDS.map(([command, what]) => (
              <li
                key={command}
                className="hairline-top flex flex-col gap-1 py-3.5 sm:flex-row sm:items-baseline sm:gap-6"
              >
                <code className="font-mono text-[13.5px] whitespace-nowrap text-ink">
                  {command}
                </code>
                <span className="text-ink-muted sm:text-right sm:ml-auto sm:max-w-[34ch]">
                  {what}
                </span>
              </li>
            ))}
